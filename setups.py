...
class Dataset:
	def __init__(self,h,w,batch_size=100,dataset_size=1000,average_sequence_length=5000,interactive=False,dt=1,L_0=1):
		self.h,self.w = h,w
		...
		self.T = torch.zeros(self.dataset_size) # timestep
	...
	def reset_env(self,index):
		...
		droll = (torch.rand(1)-0.5)*2*2*3.14*0.01#*0 # keep only roll for rotation
	...
	def ask(self):
		...
		def BoundaryConditions(x,v):
			...
			x[:,:3,0,0] = self.conditions[self.indices,0,:] + torch.sin(self.T[self.indices]*self.translation_freq[self.indices])*self.translation_amp[self.indices]
			x[:,:3,-1,0] = self.conditions[self.indices,1,:] + torch.sin(self.T[self.indices]*self.translation_freq[self.indices])*self.translation_amp[self.indices]
			v[:,:,0,0] = v[:,:,-1,0] = 0
			return x,v
